...
export default function Login() {
  ...
  const handleSubmit = (values: LoginBody, helpers: FormikHelpers<LoginBody>) => {
    trigger(values)
  }
  useEffect(() => {
    if (data) {
  ...
  return <Center>
    <Box border='1px solid #4e4e4e' borderRadius={'12px'} p='2em'>
      <Formik validationSchema={LoginBodyValidation} initialValues={initialValues} onSubmit={handleSubmit}>
        <Flex as={Form} flexDirection={'column'} gap='1em'>
          ...
          <Button isLoading={isLoading} bgColor='orange.600' type='submit'>Zaloguj</Button>
        </Flex>
      </Formik>
    </Box>
  </Center>
