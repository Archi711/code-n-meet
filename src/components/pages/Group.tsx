      ...
      <Stack>
        <Stack>
          <Stack>
            ...
            <HStack>
              <Badge colorScheme={badgeVariants[dataG.type]}>{dataG.type}</Badge>
              <Badge colorScheme={dataG.isPrivate ? "blackAlpha" : "pink"}>
                {dataG.isPrivate ? "Private" : "Public"}
              </Badge>
              <Badge colorScheme='yellow'>Active users: {dataG.Users.length}</Badge>
            </HStack>
          </Stack>
        ...
        <Grid templateColumns={'repeat(3, 1fr)'} gap={4} bg='whiteAlpha.50'>
          <GridItem colSpan={2} p={4}><GroupPosts /></GridItem>
          <GridItem p={4}><GroupUsers owner={dataG.User} users={dataG.Users} /></GridItem>
        </Grid>
      </Stack>
    )
}
