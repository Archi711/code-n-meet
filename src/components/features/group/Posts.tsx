...
export default function GroupPosts() {
  ...
  else return <Stack><Heading size='lg' as='h3'>Posts</Heading>
    <Stack>
      {
        data && data.length ? data.map((post, idx) => <PostShort post={post} key={`post-group-short-${idx}-${post.id}`} />)
          : <Stack><Text>Nobody posted!
          </Text>

            <AppLink to={`post-creator`}><Button variant='outline'>add post</Button></AppLink>
